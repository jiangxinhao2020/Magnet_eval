...
class CSAM(nn.Module):
    ...
    def forward(self, x1, x2, x3):
        ...
        x3_2 = F.interpolate(x3, scale_factor=4, mode='bilinear')

        x1_3 = self.conv_1(x1)
        x2_3 = self.conv_2(x2_2)
        x3_3 = self.conv_3(x3_2)
...
class CEM(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        self.branch1 = nn.Sequential(BasicConv2d(out_channel, out_channel, 3, padding=2, dilation=2))
        self.branch2 = nn.Sequential(BasicConv2d(out_channel, out_channel, 5, padding=4, dilation=2))
        self.branch3 = nn.Sequential(BasicConv2d(out_channel, out_channel, 7, padding=6, dilation=2))
...
class Magnet(nn.Module):
    def __init__(self, imagenet_pretrained=True):
        ...
        self.CEM2 = CEM(512, 128)
        self.CEM3 = CEM(1024, 128)
        self.CEM4 = CEM(2048, 128)

        self.conv1 = BasicConv2d(1024, 64, kernel_size=1)
        self.conv2 = BasicConv2d(384, 64, kernel_size=3, padding=1)
...
if __name__ == '__main__':
    ...
    net = Magnet(imagenet_pretrained=False).cuda()
    net.eval()
